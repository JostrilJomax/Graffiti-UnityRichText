...
/// <summary>
///     • This class and it's subclasses are designed to perform certain actions (such as insertion, replacement) to
///     char/char[]/string/StringBuilder.
///     <para/>
///     • This class also contains special symbols, that I call "modifier characters" that allow to add to a char
///     Underline/Strikethrough/etc.
/// </summary>
/// <remarks>
///     See <see cref="GraffitiStylist.AddTag"/><br/> See <see cref="GraffitiStylist.ReplaceTag"/><br/> See
...
internal static partial class GraffitiStylist {
    ...
    {
        (char[] before, char[] between, char[] after) scope = GetScope(self, startIndex, endIndex);
    ...
    [Pure] public static string Colorize(
        [NotNull] string self,
        [NotNull] Gradient gradient,
        int t,
        int length,
    ...
    [Pure] public static char[] Colorize(
        [NotNull] char[] self,
        [NotNull] Gradient gradient,
        int t,
        int length,
    ...
    private static bool IsIgnoredSymbol(char me, char[] ignoredSymbols = null)
    {
        ...
        foreach (char ignore in ignoredSymbols)
            if (me == ignore) {
                return true;
            }
    ...
    {
        char[] before = startIndex - 1 <= 0
                ? Array.Empty<char>()
                : CharsFromTo(sb, 0, startIndex - 1);

        char[] between = CharsFromTo(sb, startIndex, endIndex);

        char[] after = endIndex + 1 >= sb.Length
                ? Array.Empty<char>()
    ...
    {
        char[] array = new char[toIndex - fromIndex + 1];
        int i = 0, j = fromIndex;
