...
internal static class ColorConvertor {
    ...
    internal static Color ToUnityColor([NotNull] string color)
    {
        if (color[0] == '#') {
            ...
            switch (color.Length) {
                ...
                case 6: return default(Color);
                case 7: return ToUnityColorFromHexColor(color);
                case 8: return default(Color);
                case 9: return ToUnityColorFromHexColor(color);
            }
        }
        else if (UnityColors.TryGetColor(color, out UnityColors.NamedHexColor res)) {
    ...
    {
        if (str[0] != '#') return default(Color);

        bool isRRGGBBAA = str.Length == 9;
        if (!isRRGGBBAA && str.Length != 7) return default(Color);
    ...
    internal static Color ToUnityColorFromShortHexColor([NotNull] string str)
    {
        ...
        if (!isRGBA && str.Length != 4) return default(Color);
    ...
    internal static string ToHexColor(Color color) => ToHexColor(color.r, color.g, color.b);
    ...
    internal static string ToHexColor(byte r, byte g, byte b) => $"#{r:X2}{g:X2}{b:X2}";
    ...
    internal static string ToShortHexColor(Color color) => ToShortHexColor(color.r, color.g, color.b);
    ...
    internal static string ToShortHexColor(byte r, byte g, byte b) => $"#{RoundHex(r):X}" + $"{RoundHex(g):X}" + $"{RoundHex(b):X}";
