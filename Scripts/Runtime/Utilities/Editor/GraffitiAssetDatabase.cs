...
internal static class GraffitiAssetDatabase {
	...
	public static bool CreateAsset<T>([NotNull] T      assetInstance,
			...
			where T : UnityEngine.Object
	{
		string assetPath = GetPathByClassName(
				nameofClass: nameofClass ?? nameof(T),
	...
	/// <summary>
	/// • The following code tries to get path (3) from path (1): <br/>
	/// (1) .../RootFolder/.../ClassName.cs <br/>
	/// (2) .../RootFolder <br/>
	/// (3) .../RootFolder/ ..RelativePath.. /AssetName.asset <br/> <br/>

	/// • How exactly it works: <br/>
	///  It finds path (1) by searching all directories for a NameofClass.cs file,
	/// then removes from the path (1) everything up to the root folder,
	/// as in path (2), then adds provided RelativePath and AssetName
	/// with .asset extension. <br/> <br/>

	/// • Why are we searching by class name ? <br/>
	/// 1) We can be somewhat sure that a file has the same name as the class it
	/// contains, so with help of the IDE and the nameof() keyword, the link can
	/// be made quite strong. <br/>
	/// 2) Folders can have duplicate names, with classes there is at least some
	/// limitation: classes cannot have the same name in the same namespace. <br/> <br/>

	/// • What happens if I have a class with the same name somewhere else? <br/>
	///  Then both classes will be ignored and asset path will be empty <br/> <br/>

	/// • What happens if I put plugin's folder in folder with the same name? <br/>
	...
	/// </summary>
	/// <param name="rootFolder"> The name of a root folder with no slashes. <example> "Root Folder" </example> </param>
	/// <param name="relativePath"> Additional path after foot folder, should start with a slash and should end with no slash, you can pass null here. <example> "/Resources" </example> </param>
	/// <param name="nameofClass"> Should not end with extension. The class name should be as unique as possible. <example> nameof(IThinkThisClassWontBeRepeatedAnywhereInTheProject) </example> </param>
	/// <param name="assetName"> Name of a ScriptableObject (asset) that you want to create. No slashes, no extensions. <example> "My Brand New Asset" </example> </param>
	/// <returns>  </returns>
	[NotNull, Pure]
	public static string GetPathByClassName([NotNull]   string nameofClass,
	                                        [NotNull]   string rootFolder,
	                                        [CanBeNull] string relativePath,
	                                        [NotNull]   string assetName)
	{
