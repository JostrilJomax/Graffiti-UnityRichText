...
public static class StyledStringExtensions {
	...
	/// <para/>
	/// • • • List of modifiers you can use (modifier can be used simultaneously):<br/><br/>
	/// 1) <b>Color</b> ( .Blue, .Yellow, etc. )<br/>
	/// 2) <b>Gradient</b> ( Just write two colors, like this: .Yellow.Blue )<br/>
	/// 3) <b>Size</b> ( .Size(24), default size is 12 )<br/>
	/// 4) <b>Font Style</b> ( .Bold, .Italic )<br/>
	/// 4) <b>Modifier characters</b> ( .Underline, .Strikethrough, etc. )
	/// <para/>
	/// • • • You can even stylize modifier characters. For example, you can create <b>Blue text with Yellow Underline</b>:
	/// <code> "Cool Text".Stylize().Blue.Underline[Style.Yellow]; </code><br/>
	///
	/// • • • You can also <b>change color brightness</b> using <b>.Dark</b>/<b>.Light</b> and color afterwards:
	/// <code> "Cool Text".Stylize().Light.Yellow; </code><br/>
	///
	/// • • • If you only want to add a modifier(s) to a specific part of a string, here is a list of ways to do so:<br/><br/>
	/// 1) <b>To One Word</b>: <b>.Style(0)</b>, where 0 - index of the word in the string you are modifying (starting from 0)<br/><br/>
	/// 2) <b>To a Range of Words</b>: <b>.Style(0, 1)</b>, where 0 - index of the word to start with, 1 - index of the word to end on<br/>
	/// (negative values are counted as 0, values bigger than the index of last word in a string are clamped)<br/><br/>
	/// 3) <b>To a Range of words v2</b>: <b>.Style(0..1)</b>, where 0..1 - Range, read more here: https://docs.microsoft.com/en-us/dotnet/api/system.range?view=net-6.0<br/><br/>
	/// 4) <b>To a Percentage</b>: <b>.Stylize(.5f)</b>, where .5f - means 50% of all words in the string<br/>
	/// (if you use negative value, words will be modified from the end)<br/><br/>
	///
	/// • • • In the following example <b>the second and the third words are colored Yellow:</b>
	...
	public static StyledString Stylize(this string self, int index, bool fromEnd = false,                    StringStyle stl = null) {stl ??= new StringStyle(); stl.PrepareScope(index, fromEnd);                         return new StyledString(self, stl);} /// <inheritdoc cref="Stylize(string,Graffiti.StringStyle)"/>
	public static StyledString Stylize(this string self, int startIndex, int endIndex, bool fromEnd = false, StringStyle stl = null) {stl ??= new StringStyle(); stl.PrepareScope(startIndex, fromEnd, endIndex, fromEnd); return new StyledString(self, stl);} /// <inheritdoc cref="Stylize(string,Graffiti.StringStyle)"/>
	...
	public static StyledString Stylize<T>(this T self, Range range,                                        StringStyle stl = null) => self.ToString().Stylize(range,                         stl); /// <inheritdoc cref="Stylize(string,Graffiti.StringStyle)"/>
	...
	public static StyledString And(this StyledString self, int index, bool fromEnd = false,                     StringStyle stl = null) { stl ??= new StringStyle(); stl.PrepareScope(index, fromEnd);                         self.Styles.Add(stl); return self; } /// <inheritdoc cref="And(StyledString,StringStyle)"/>
	public static StyledString And(this StyledString self, int startIndex, int endIndex, bool fromEnd = false,  StringStyle stl = null) { stl ??= new StringStyle(); stl.PrepareScope(startIndex, fromEnd, endIndex, fromEnd); self.Styles.Add(stl); return self; } /// <inheritdoc cref="And(StyledString,StringStyle)"/>
