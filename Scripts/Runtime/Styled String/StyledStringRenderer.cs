...
internal static class StyledStringRenderer {
    ...
    internal static StringBuilder Render([NotNull] StringStyle style, [NotNull] string str)
    {
        ...
        var styleScope = new Scope(style.Scope.CreateScope(elementBorders.Length));
    ...
    internal static StringBuilder Render([NotNull] StringStyle[] styles, [NotNull] string str)
    {
        ...
        var cachedStyleScopes = new Scope[styles.Length];
    ...
    private static Scope[] GetWords(string str, char[] separators)
    {
        ...
        for (; i < str.Length; i++) {
            ...
            // Found a word
            var tempScope = new Scope { Start = separatorStartIndex };
        ...
        char[] ignoredSymbols)
    {
        ...
        if (style.HasModifierCharacterSet) {
            ...
            GraffitiStylist.AddGradient(
                sb, style.Gradient, start, end, gradientSimulationStart,
                gradientSimulationEnd);
        }

        if (style.HasOnlyOneColor) {
            GraffitiStylist.AddTag.Color(sb, style.Color.GetColorHexValue(), start, AdjustedEndIndex());
        }
    ...
    private struct Scope {
        ...
        public Scope((int, int) tuple)
        {
            Start = tuple.Item1;
            End = tuple.Item2;
        }

        public Scope(int item1, int item2)
        {
            Start = item1;
            End = item2;
        }
